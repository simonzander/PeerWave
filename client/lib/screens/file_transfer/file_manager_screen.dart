...
class _FileManagerScreenState extends State<FileManagerScreen> {
  ...
  Map<String, Map<String, dynamic>> _seedingStats =
      {}; // fileId -> {downloaders, transferRate}
  ...
  FileStorageInterface _getStorage() {
    if (_storage == null) {
      ...
      if (storage == null) {
        throw Exception(
          'FileStorageInterface not initialized. Please login first.',
        );
      }
      _storage = storage;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final storageAvailable =
        Provider.of<FileStorageInterface?>(context, listen: true) != null;
  ...
  Widget _buildFilterChip(String label, FileFilter filter, IconData icon) {
    ...
    return FilterChip(
      ...
      labelStyle: TextStyle(color: isSelected ? colorScheme.onPrimary : null),
    );
  }
  ...
  Widget _buildFileCard(Map<String, dynamic> file) {
    ...
    final isComplete =
        status == 'complete' ||
        status == 'seeding' ||
        downloadedChunksCount == chunkCount;
    ...
    final isPartialSeeder =
        status == 'downloading' && downloadedChunksCount > 0 && isSeeder;
      ...
      for (final file in files) {
        ...
        if (fileId != null) {
          ...
          final enrichedFile = {
            ...file,
            'downloadedChunks':
                availableChunks.length, // Add actual downloaded chunk count
            'availableChunksList':
                availableChunks, // Keep the list for later use
          };
...
class _AddFileProgressDialogState extends State<_AddFileProgressDialog> {
  ...
  Future<void> _processFile() async {
    try {
      ...
      final fileChecksum = chunkingService.calculateFileChecksum(
        widget.fileBytes,
      );
      ...
      final duplicate = existingFiles.firstWhere(
        (f) =>
            f['fileName'] == widget.fileName && f['checksum'] == fileChecksum,
        orElse: () => {},
      );
      ...
      debugPrint(
        '[ADD_FILE] Generated AES-256 file key: ${fileKey.length} bytes',
      );
      ...
      debugPrint(
        '[ADD_FILE] Saved file key to storage: ${fileKey.length} bytes',
      );
      ...
      try {
        ...
        if (socketService.socket != null) {
          ...
          if (result['sharedWith'] != null) {
            ...
            debugPrint(
              '[ADD_FILE] Updated sharedWith from server: ${result['sharedWith'].length} users',
            );
          }

          debugPrint(
            '[ADD_FILE] File announced: $fileId with ${chunks.length} chunks',
          );
        } else {
          debugPrint(
            '[ADD_FILE] WARNING: Socket not connected, file not announced',
          );
        }
      } catch (e) {
...
/// File Filter Options
enum FileFilter { all, myFiles, downloads, seeding }
...
class _ShareFileDialogState extends State<_ShareFileDialog> {
  ...
  Future<void> _shareFile() async {
    ...
    try {
      ...
      if (socketService.socket == null) {
        throw Exception(
          'Socket not connected. Please ensure you are connected to the server.',
        );
      }
    ...
    return AlertDialog(
      title: Row(
        children: [
          ...
          const SizedBox(width: 12),
          Expanded(child: Text('Share: ${widget.file['fileName']}')),
        ],
      ),
      content: SizedBox(
        width: 500,
...
class _DownloadFileProgressDialog extends StatefulWidget {
  ...
  @override
  State<_DownloadFileProgressDialog> createState() =>
      _DownloadFileProgressDialogState();
}

class _DownloadFileProgressDialogState
    extends State<_DownloadFileProgressDialog> {
  double _progress = 0.0;
  String _statusText = 'Preparing...';
  ...
  Future<void> _downloadFile() async {
    try {
      ...
      for (int i = 0; i < widget.downloadedChunks.length; i++) {
        ...
        final encryptedBytes = await widget.storage.getChunk(
          widget.fileId,
          chunkIndex,
        );
        final chunkMetadata = await widget.storage.getChunkMetadata(
          widget.fileId,
          chunkIndex,
        );
