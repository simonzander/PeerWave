...
class _FileManagerScreenState extends State<FileManagerScreen> {
  ...
  Future<void> _loadLocalFiles() async {
    ...
    try {
      ...
      // ========================================
      // FALLBACK: Verify downloading files
      // ========================================
      // Check if files marked as "downloading" are actually complete
      // and verify checksums to fix interrupted downloads
      
      for (final file in files) {
        final status = file['status'] as String?;
        final fileId = file['fileId'] as String?;
        final chunkCount = file['chunkCount'] as int? ?? 0;
        final expectedChecksum = file['checksum'] as String?;
        
        if (fileId == null || chunkCount == 0) continue;
        
        // Only check files that are marked as "downloading"
        if (status == 'downloading') {
          try {
            ...
            if (availableChunks.length == chunkCount) {
              print('[FILE_MANAGER] File $fileId has all chunks (${availableChunks.length}/$chunkCount), verifying checksum...');
              
              // Verify checksum
              bool isChecksumValid = false;
              
              if (expectedChecksum != null) {
                isChecksumValid = await _verifyFileChecksum(fileId, expectedChecksum);
              }
              
              if (isChecksumValid) {
                // File is complete and checksum matches - update status
                print('[FILE_MANAGER] ✓ Checksum verified for $fileId - marking as complete');
                ...
                file['downloadedChunks'] = availableChunks;
              } else {
                // Checksum mismatch - file is corrupted
                print('[FILE_MANAGER] ⚠ Checksum mismatch for $fileId - file corrupted');
                
                // Show dialog to user (after loading is complete)
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  if (mounted) {
                    _showCorruptedFileDialog(
              ...
              print('[FILE_MANAGER] File $fileId incomplete: ${availableChunks.length}/$chunkCount chunks');
            }
          } catch (e) {
            print('[FILE_MANAGER] Error verifying file $fileId: $e');
          }
        }
      }
  ...
  /// Verify file checksum by assembling and hashing all chunks
  Future<bool> _verifyFileChecksum(String fileId, String expectedChecksum) async {
    try {
      final storage = _getStorage();
      ...
      if (fileKey == null) {
        print('[FILE_MANAGER] No file key found for $fileId');
        return false;
      }
      ...
      for (final chunkIndex in availableChunks) {
        ...
        final chunkMetadata = await storage.getChunkMetadata(fileId, chunkIndex);
        
        if (encryptedBytes == null || chunkMetadata == null) {
          print('[FILE_MANAGER] Chunk $chunkIndex missing');
          return false;
        }
        
        final iv = chunkMetadata['iv'] as Uint8List?;
        if (iv == null) {
          print('[FILE_MANAGER] IV missing for chunk $chunkIndex');
          return false;
        }
        ...
        if (decryptedBytes == null) {
          print('[FILE_MANAGER] Failed to decrypt chunk $chunkIndex');
          return false;
        }
      ...
      // Calculate checksum
      final actualChecksum = chunkingService.calculateFileChecksum(fileBytes);
      
      print('[FILE_MANAGER] Expected: $expectedChecksum');
      print('[FILE_MANAGER] Actual:   $actualChecksum');
      
      return actualChecksum == expectedChecksum;
      
    } catch (e) {
      print('[FILE_MANAGER] Error verifying checksum: $e');
      return false;
    }
  }
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            ...
            const Expanded(child: Text('Corrupted File Detected')),
          ],
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          ...
          children: [
            Text(
              'The file "$fileName" appears to be corrupted:',
              style: const TextStyle(fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 12),
            const Text('• All chunks are present'),
            const Text('• But checksum verification failed'),
            const SizedBox(height: 16),
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
              ...
              child: const Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'This could mean the file was tampered with or corrupted during download.',
            ...
            const SizedBox(height: 16),
            const Text(
              'Would you like to delete the corrupted chunks and re-download the file?',
        ...
        actions: [
          ...
          ElevatedButton.icon(
            onPressed: () {
              Navigator.pop(context);
              _deleteCorruptedFile(fileId, fileName);
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.orange[700],
            ),
            icon: const Icon(Icons.delete_sweep),
            label: const Text('Delete & Re-download'),
          ),
        ],
      ),
    );
  ...
  Future<void> _deleteCorruptedFile(String fileId, String fileName) async {
    try {
      final storage = _getStorage();
      
      print('[FILE_MANAGER] Deleting corrupted chunks for: $fileId');
      
      // Get file metadata before deleting chunks
      ...
      // Delete only the chunks, keep metadata
      for (final chunkIndex in availableChunks) {
        await storage.deleteChunk(fileId, chunkIndex);
        print('[FILE_MANAGER] Deleted chunk $chunkIndex');
      }
      
      // Reset file status to allow re-download
      await storage.updateFileMetadata(fileId, {
        'status': 'downloading', // Start re-download
        'isSeeder': false,
        'downloadComplete': false,
        'downloadedChunks': <int>[], // Empty list
        'progress': 0.0,
      });
      
      print('[FILE_MANAGER] Reset metadata for $fileId - starting re-download');
      
      _showSuccess('Corrupted chunks deleted for "$fileName". Starting re-download...');
      
      // Reload file list to show updated status
      await _loadLocalFiles();
      
      // ========================================
      // OPTION 3: Automatic Re-Download
      // ========================================
      // Start automatic re-download using FileTransferService
      ...
      try {
        ...
        print('[FILE_MANAGER] Starting automatic re-download for $fileId');
        
        // Start download with progress callback
        await fileTransferService.downloadFile(
          fileId: fileId,
          onProgress: (progress) {
            print('[FILE_MANAGER] Re-download progress: ${(progress * 100).toStringAsFixed(1)}%');
          },
          allowPartial: true, // Allow partial downloads
        );
        
        print('[FILE_MANAGER] ✓ Re-download complete for $fileId');
        
        // Reload file list to show completed download
        _loadLocalFiles();
        
        _showSuccess('File "$fileName" successfully re-downloaded and verified!');
        
      } catch (e) {
        print('[FILE_MANAGER] Error during re-download: $e');
        _showError('Failed to re-download file: $e');
      ...
      print('[FILE_MANAGER] Error deleting corrupted chunks: $e');
      _showError('Failed to delete corrupted chunks: $e');
    }
  }
