...
class AppDirectories {
    ...
    try {
      ...
      if (Platform.isWindows) {
        ...
        if (appData != null) {
          ...
          _appDataDir = Directory(
            path.join(temp.parent.path, 'Local', _appName),
          );
        }
        ...
        _appDataDir = Directory(
          path.join(appData.parent.path, '.local', 'share', _appName),
        );
      } else {
      ...
      if (!await _appDataDir!.exists()) {
        ...
        debugPrint(
          '[AppDirectories] Created main directory: ${_appDataDir!.path}',
        );
      }
    ...
    try {
      ...
      if (await oldDbFile.exists()) {
        ...
        if (!await newDbFile.exists()) {
          debugPrint(
            '[AppDirectories] Migrating database from $oldDbPath to $newDbPath',
          );
      ...
      if (await oldChunksDir.exists()) {
        final newChunksDir = Directory(
          path.join(_cacheDir!.path, 'file_chunks'),
        );
        if (!await newChunksDir.exists()) {
          debugPrint(
            '[AppDirectories] Migrating file_chunks from ${oldChunksDir.path} to ${newChunksDir.path}',
          );
  ...
  static Directory get appDataDirectory {
    if (_appDataDir == null) {
      throw StateError(
        'AppDirectories not initialized. Call initialize() first.',
      );
    }
    return _appDataDir!;
  }
