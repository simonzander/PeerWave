...
class FileTransferStats {
  ...
  FileTransferStats({required this.fileId, required this.fileName});
}
...
class FileTransferStatsProvider extends ChangeNotifier {
  ...
  /// Get active uploads
  List<ActiveTransfer> get activeUploads => _activeTransfers.values
      .where((t) => t.direction == TransferDirection.upload)
      .toList();

  /// Get active downloads
  List<ActiveTransfer> get activeDownloads => _activeTransfers.values
  ...
  void _calculateSpeeds() {
    ...
    final uploadBytesDelta =
        currentUploadBytes - (_lastUploadBytes['total'] ?? 0);
    ...
    final downloadBytesDelta =
        currentDownloadBytes - (_lastDownloadBytes['total'] ?? 0);
