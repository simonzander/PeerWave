...
class GuestMeetingVideoConferenceView extends StatefulWidget {
  final String meetingId;
  final String meetingTitle;
  final lk.MediaDevice? selectedCamera;
  final lk.MediaDevice? selectedMicrophone;

  const GuestMeetingVideoConferenceView({
    super.key,
  ...
  @override
  State<GuestMeetingVideoConferenceView> createState() =>
      _GuestMeetingVideoConferenceViewState();
}

class _GuestMeetingVideoConferenceViewState
    extends State<GuestMeetingVideoConferenceView> {
  ...
  void didChangeDependencies() {
    ...
    if (_service == null) {
      try {
        ...
        debugPrint('[GuestMeetingVideo] Failed to get service: $e');
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            setState(() {
  ...
  Future<void> _joinMeeting() async {
    ...
    try {
      debugPrint('[GuestMeetingVideo] Joining meeting: ${widget.meetingId}');

      // Get guest session ID from storage (web only)
      String? guestSessionId;
      if (kIsWeb) {
        try {
          guestSessionId = getSessionStorageItem('external_session_id');
          debugPrint('[GuestMeetingVideo] Guest session ID: $guestSessionId');
        } catch (e) {
          debugPrint('[GuestMeetingVideo] Error getting session ID: $e');
        }
      }
      ...
      await _service!.joinRoom(
        ...
        isExternalGuest: true, // Skip Signal Protocol checks
        guestSessionId: guestSessionId, // For LiveKit token request
      );

      setState(() => _isJoining = false);
      debugPrint('[GuestMeetingVideo] Successfully joined meeting');
      ...
      debugPrint('[GuestMeetingVideo] Join error: $e');
      setState(() {
        _isJoining = false;
  ...
  Future<void> _leaveMeeting() async {
    ...
    try {
      ...
      // For guests, just show a message (they have no app to navigate to)
      if (mounted) {
        showDialog(
          context: context,
      ...
      debugPrint('[GuestMeetingVideo] Leave error: $e');
    }
  }
  ...
  void dispose() {
    ...
    if (_service != null && _service!.isInCall) {
      debugPrint('[GuestMeetingVideo] Exiting full-view');
      WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  int _calculateMaxVisible(Size screenSize, bool hasScreenShare) {
    if (hasScreenShare) {
      ...
      if (area > 1920 * 1080) return 12;
      if (area > 1280 * 720) return 9;
      return 6;
    }
  }
  ...
  void _loadParticipantProfile(String participantId) {
    // Guests see simplified names (no profile loading from backend)
    _displayNameCache[participantId] = 'Participant';
