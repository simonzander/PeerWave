...
class WebAuthnCryptoService {
  ...
  Future<Uint8List> deriveEncryptionKey(Uint8List signature) async {
    debugPrint('[WEBAUTHN_CRYPTO] Deriving encryption key from signature (${signature.length} bytes)');
  ...
  Uint8List _deriveKeyHKDF(Uint8List ikm, List<int> salt, List<int> info) {
    ...
    debugPrint('[WEBAUTHN_CRYPTO] ✓ Key derived successfully (${key.length} bytes)');
    return key;
  }
  ...
  Future<Map<String, String>> encrypt(Uint8List plaintext, Uint8List key) async {
    ...
    try {
      ...
      debugPrint('[WEBAUTHN_CRYPTO] ✓ Data encrypted (${encrypted.length} bytes)');
  ...
  Future<Uint8List> decrypt(String ivBase64, String dataBase64, Uint8List key) async {
    ...
    try {
      ...
      debugPrint('[WEBAUTHN_CRYPTO] ✓ Data decrypted (${decrypted.length} bytes)');
