...
class StarredChannelsService {
  static final StarredChannelsService _instance =
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _starredChannels = results
          .map((row) => row['channel_uuid'] as String)
          .toSet();
      _initialized = true;

      debugPrint(
        '[STARRED] Initialized with ${_starredChannels.length} starred channels',
      );
    } catch (e) {
  ...
  List<String> getStarredChannels() {
    if (!_initialized) {
      debugPrint(
        '[STARRED] Warning: Service not initialized, returning empty list',
      );
      return [];
    }
