...
abstract class FileStorageInterface {
  ...
  Future<bool> saveChunkSafe(
    ...
    if (existingChunk != null && existingChunk.length == encryptedData.length) {
      debugPrint(
        '[STORAGE] Chunk $chunkIndex already exists (${existingChunk.length} bytes), skipping duplicate',
      );
      return false;
    }
    ...
    if (existingChunk != null) {
      debugPrint(
        '[STORAGE] ⚠️ Chunk $chunkIndex size mismatch: ${existingChunk.length} != ${encryptedData.length}, overwriting',
      );
    }
    ...
    debugPrint(
      '[STORAGE] ✓ Chunk $chunkIndex saved safely (${encryptedData.length} bytes)',
    );
    return true;
  }
...
class FileMetadata {
  ...
  final String chatId;
  final String
  status; // 'uploading' | 'uploaded' | 'downloading' | 'complete' | 'paused' | 'failed'
  ...
  final int?
  lastSync; // Last sync timestamp with server (milliseconds since epoch)
