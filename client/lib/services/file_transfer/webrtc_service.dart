...
class WebRTCFileService extends ChangeNotifier {
  ...
  WebRTCFileService({Map<String, dynamic>? iceServers})
  ...
  Future<RTCPeerConnection> _createPeerConnectionForPeer(String peerId) async {
    ...
    pc.onIceCandidate = (candidate) {
      debugPrint(
        '[WebRTC ICE] Candidate discovered for $peerId: ${candidate.candidate?.substring(0, 50) ?? "null"}...',
      );
      if (_iceCandidateCallback == null) {
        debugPrint(
          '[WebRTC ICE] ❌ WARNING: No ICE candidate callback registered!',
        );
      } else {
  ...
  Future<void> sendData(String peerId, dynamic data) async {
    ...
    debugPrint(
      '[WebRTC] Available data channels: ${_dataChannels.keys.toList()}',
    );
    ...
    if (channel.state != RTCDataChannelState.RTCDataChannelOpen) {
      debugPrint(
        '[WebRTC] ERROR: Data channel not open for $peerId: ${channel.state}',
      );
  ...
  Future<void> sendBinary(String peerId, Uint8List data) async {
    ...
    while (channel.bufferedAmount != null &&
      ...
      if (waitCount % 100 == 0) {
        debugPrint(
          '[WebRTC Backpressure] $peerId: Waiting for buffer to drain: '
    ...
    if (channel.bufferedAmount != null) {
      ...
      if (channel.bufferedAmount! > lowWaterMark) {
        debugPrint(
          '[WebRTC Buffer] $peerId: ${(channel.bufferedAmount! / 1024 / 1024).toStringAsFixed(2)} MB buffered',
        );
      }
    }
  ...
  void onMessage(
    String peerId,
    Function(String peerId, dynamic data) callback,
  ) {
    debugPrint('[WebRTC] Registering message callback for $peerId');
    _messageCallbacks[peerId] = callback;
    debugPrint(
      '[WebRTC] Message callback registered. Total callbacks: ${_messageCallbacks.length}',
    );
  }
  ...
  List<String> getConnectedPeers() {
    return _connectionStates.entries
        .where(
          (e) =>
              e.value == RTCPeerConnectionState.RTCPeerConnectionStateConnected,
        )
        .map((e) => e.key)
        .toList();
  }
  ...
  void _setupDataChannel(String peerId, RTCDataChannel channel) {
    ...
    channel.onDataChannelState = (state) {
      ...
      if (state == RTCDataChannelState.RTCDataChannelOpen) {
        debugPrint(
          '[WebRTC] Data channel $peerId is now OPEN, calling callback',
        );
  ...
  void _onDataChannelMessage(String peerId, RTCDataChannelMessage message) {
    try {
      debugPrint(
        '[WebRTC] _onDataChannelMessage called for $peerId, isBinary: ${message.isBinary}',
      );

      if (message.isBinary) {
        // Binary data (chunk)
        debugPrint(
          '[WebRTC] Calling binary callback for $peerId (${message.binary.length} bytes)',
        );
        _messageCallbacks[peerId]?.call(peerId, message.binary);
        if (_messageCallbacks[peerId] == null) {
          debugPrint(
            '[WebRTC] WARNING: No message callback registered for $peerId',
          );
        }
      } else {
        // Text data (JSON)
  ...
  void setIceCandidateCallback(
    Function(String peerId, RTCIceCandidate candidate) callback,
  ) {
...
class BufferStats {
  ...
  void recordWait(int bufferedAmount, Duration waitTime) {
    totalWaits++;
    maxBufferedAmount = maxBufferedAmount > bufferedAmount
        ? maxBufferedAmount
        : bufferedAmount;
  ...
  void recordTimeout(int bufferedAmount) {
    timeouts++;
    debugPrint(
      '[BufferStats] ❌ TIMEOUT! Buffer stuck at ${(bufferedAmount / 1024 / 1024).toStringAsFixed(2)} MB',
    );
  }

  void printStats() {
    final avgWaitMs = totalWaits > 0
        ? totalWaitTime.inMilliseconds / totalWaits
        : 0;
    ...
    if (totalWaits > totalSends * 0.1) {
      debugPrint(
        '  ⚠️  High backpressure detected (${((totalWaits / totalSends) * 100).toStringAsFixed(1)}% of sends)',
      );
    }
  }
}
