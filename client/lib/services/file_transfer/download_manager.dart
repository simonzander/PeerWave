...
class DownloadManager extends ChangeNotifier {
  ...
  Future<void> _downloadChunk(String fileId, int chunkIndex, DownloadTask task) async {
    int retries = 0;
    ...
    while (retries < maxRetries) {
      ...
      try {
        ...
        final encryptedChunk = await _requestChunkFromPeer(seeder, fileId, chunkIndex);
        ...
        if (retries >= maxRetries) {
          throw Exception('Failed to download chunk $chunkIndex after $maxRetries attempts: $e');
        }
  ...
  String? _findSeederForChunk(Map<String, List<int>> seederChunks, int chunkIndex) {
  ...
  Future<Uint8List?> _requestChunkFromPeer(String peerId, String fileId, int chunkIndex) async {
...
class DownloadTask {
  ...
  double get progress => chunkCount > 0 ? (downloadedChunks / chunkCount) * 100 : 0;
