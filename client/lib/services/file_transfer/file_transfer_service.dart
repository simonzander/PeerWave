...
class FileTransferService {
  ...
  Future<String> uploadAndAnnounceFile({
    ...
    try {
      debugPrint(
        '[FILE TRANSFER] Starting upload: $fileName (${fileBytes.length} bytes)',
      );
      ...
      debugPrint(
        '[FILE TRANSFER] Generated AES-256 file key: ${fileKey.length} bytes',
      );
      ...
      debugPrint(
        '[FILE TRANSFER] Encrypted and stored ${chunks.length} chunks',
      );
      ...
      debugPrint(
        '[FILE TRANSFER] Saved file key to storage: ${fileKey.length} bytes',
      );
  ...
  Future<void> reannounceUploadedFiles() async {
    try {
      ...
      debugPrint(
        '[FILE TRANSFER] Found ${uploadedFiles.length} files to re-announce (including partial downloads)',
      );
      ...
      for (final file in uploadedFiles) {
        ...
        try {
          ...
          if (availableChunks.isEmpty) {
            debugPrint(
              '[FILE TRANSFER] Warning: No chunks found for $fileId, skipping',
            );
            continue;
          }
          ...
          final sharedWith =
              (file['sharedWith'] as List?)?.cast<String>() ?? [];
          final status = file['status'] as String;

          debugPrint(
            '[FILE TRANSFER] Re-announcing: $fileId (${availableChunks.length} chunks, status: $status)',
          );
          ...
          try {
            debugPrint(
              '[FILE TRANSFER] HIGH #2: Syncing state with server for $fileId',
            );

            // Query server for current file state
            final fileInfo = await _socketFileClient.getFileInfo(fileId);
            final serverSharedWith =
                (fileInfo['sharedWith'] as List?)?.cast<String>() ?? [];
            ...
            debugPrint(
              '[FILE TRANSFER] ✓ State synced: $fileId (${availableChunks.length}/${file['chunkCount']} chunks) shared with ${serverSharedWith.length} users',
            );
          } catch (e) {
            debugPrint(
              '[FILE TRANSFER] ⚠ Failed to sync state for $fileId: $e',
            );
  ...
  Future<void> downloadFile({
    ...
    try {
      debugPrint(
        '[FILE TRANSFER] Starting download: $fileId (partial: $allowPartial)',
      );
      ...
      if (chunkQuality < 100 && !allowPartial) {
        throw Exception(
          'File incomplete ($chunkQuality% available). Enable partial downloads.',
        );
      }
      ...
      debugPrint(
        '[FILE TRANSFER] Step 2.5: Saving initial metadata and announcing...',
      );
      ...
      try {
        ...
        debugPrint(
          '[FILE TRANSFER] ✓ Announced as seeder with 0 chunks (downloading)',
        );
      } catch (e) {
        debugPrint(
          '[FILE TRANSFER] Warning: Could not save metadata or announce: $e',
        );
      ...
      for (int i = 0; i < totalChunks; i++) {
        ...
        if (isChunkValid) {
          ...
          debugPrint(
            '[FILE TRANSFER] ⚠️ Chunk $i failed verification - not announcing',
          );
      ...
      final allChunksVerified =
          verifiedChunks.length == downloadedChunks.length;

      // Step 7: Verify complete file checksum if download is complete
      if (isComplete && allChunksVerified) {
        debugPrint(
          '[FILE TRANSFER] Step 7: Verifying complete file checksum...',
        );
        final isValid = await _verifyFileChecksum(fileId);

        if (!isValid) {
          debugPrint(
            '[SECURITY] ❌ File checksum verification FAILED! File corrupted or tampered.',
          );
          ...
          if (corruptedChunks.isEmpty) {
            ...
            debugPrint(
              '[FILE TRANSFER] ⚠️ No corrupted chunks found, but checksum fails',
            );
            debugPrint(
              '[FILE TRANSFER] Deleting entire file for clean re-download',
            );
            await _deleteCorruptedFile(fileId);
            throw Exception(
              'File integrity check failed - checksum mismatch (metadata corrupted)',
            );
          }
          ...
          debugPrint(
            '[FILE TRANSFER] ✓ Corrupted chunks marked for re-download',
          );
          debugPrint(
            '[FILE TRANSFER] File will automatically re-download ${corruptedChunks.length} chunks',
          );
          ...
          debugPrint(
            '[FILE TRANSFER] Announcing verified chunks during recovery...',
          );
          ...
          try {
            ...
            if (metadata != null) {
              ...
              debugPrint(
                '[FILE TRANSFER] ✓ Announced ${verifiedChunks.length} verified chunks during recovery',
              );
            }
          } catch (e) {
            debugPrint(
              '[FILE TRANSFER] Warning: Could not announce during recovery: $e',
            );
          }
        ...
        debugPrint(
          '[FILE TRANSFER] ✓ File checksum verified - file is authentic',
        );
      } else if (!allChunksVerified) {
        debugPrint(
          '[FILE TRANSFER] ⚠️ Some chunks failed verification (${downloadedChunks.length - verifiedChunks.length} failed)',
        );
      }
      ...
      await _storage.updateFileMetadata(fileId, {
        ...
        'isSeeder': true,
        'downloadedChunks':
            verifiedChunks, // Use verifiedChunks instead of downloadedChunks
      });
      ...
      try {
        ...
        if (metadata != null) {
          ...
          debugPrint(
            '[FILE TRANSFER] ✓ Announced as seeder with ${verifiedChunks.length}/$totalChunks verified chunks',
          );
        }
      } catch (e) {
      ...
      if (isComplete && allChunksVerified) {
        ...
        debugPrint(
          '[FILE TRANSFER] ⚠ Partial download: $fileId (${verifiedChunks.length}/$totalChunks verified chunks)',
        );
      }
    } catch (e) {
  ...
  Future<void> resumeIncompleteDownloads() async {
    try {
      ...
      final allFiles = await _storage.getAllFiles();
      final incompleteFiles = allFiles
          .where(
            (file) =>
      ...
      debugPrint(
        '[FILE TRANSFER] Found ${incompleteFiles.length} incomplete downloads',
      );
      ...
      for (final file in incompleteFiles) {
        ...
        try {
          ...
          if (chunkQuality > 0) {
            debugPrint(
              '[FILE TRANSFER] Resuming download: $fileId (quality: $chunkQuality%)',
            );
            ...
            unawaited(
              downloadFile(
                ...
                onProgress: (progress) {
                  debugPrint(
                    '[FILE TRANSFER] Resume progress for $fileId: ${(progress * 100).toInt()}%',
                  );
                },
                allowPartial: true,
              ).catchError((e) {
            ...
            debugPrint(
              '[FILE TRANSFER] No chunks available for $fileId, skipping',
            );
          }
        } catch (e) {
  ...
  void _setupAnnounceListener() {
    ...
    Future<void> checkAndResumeDownload(String fileId, int chunkQuality) async {
      try {
        ...
        debugPrint(
          '[FILE TRANSFER] Found incomplete download for $fileId, checking for new chunks',
        );

        // Get our downloaded chunks
        final ourChunks =
            (metadata['downloadedChunks'] as List?)?.cast<int>() ?? [];
        ...
        final newChunks = availableChunks
            .where((idx) => !ourChunks.contains(idx))
            .toList();

        if (newChunks.isNotEmpty) {
          debugPrint(
            '[FILE TRANSFER] Found ${newChunks.length} new chunks, auto-resuming download for $fileId',
          );
          ...
          unawaited(
            downloadFile(
              ...
              onProgress: (progress) {
                debugPrint(
                  '[FILE TRANSFER] Auto-resume progress: ${(progress * 100).toInt()}%',
                );
              },
              allowPartial: true,
            ).catchError((e) {
    ...
    _socketFileClient.onFileAnnounced((data) async {
      ...
      debugPrint(
        '[FILE TRANSFER] File announced: $fileId (quality: $chunkQuality%)',
      );
      ...
      if (newSharedWith.isNotEmpty) {
        ...
        if (existingMetadata != null) {
          final existingSharedWith =
              (existingMetadata['sharedWith'] as List?)?.cast<String>() ?? [];

          // Merge: combine both lists and remove duplicates
          final mergedSharedWith = <String>{
            ...existingSharedWith,
            ...newSharedWith,
          }.toList();
          ...
          if (mergedSharedWith.length != existingSharedWith.length) {
            debugPrint(
              '[FILE TRANSFER] Merging sharedWith: ${existingSharedWith.length} -> ${mergedSharedWith.length} users',
            );
    ...
    _socketFileClient.onFileSeederUpdate((data) async {
      ...
      debugPrint(
        '[FILE TRANSFER] Seeder update: $fileId (quality: $chunkQuality%)',
      );
      await checkAndResumeDownload(fileId, chunkQuality);
    });
  }
  ...
  Future<void> addUsersToShare({
    ...
    try {
      ...
      debugPrint(
        '[FILE TRANSFER] ✓ Server updated: ${serverUpdate['successCount']} users added',
      );
      ...
      if (chatType == 'direct' && userIds.length == 1) {
        ...
        debugPrint(
          '[FILE TRANSFER] - metadata: ${metadata != null ? "present" : "null"}',
        );
        ...
        if (metadata != null) {
          ...
          debugPrint(
            '[FILE TRANSFER] ⚠️ Metadata is null, skipping group message',
          );
        }
      }
      ...
      debugPrint(
        '[FILE TRANSFER] ✓ Added ${userIds.length} users to share: $fileId',
      );
    } catch (e) {
  ...
  Future<void> revokeUsersFromShare({
    ...
    try {
      ...
      debugPrint(
        '[FILE TRANSFER] ✓ Server updated: ${serverUpdate['successCount']} users revoked',
      );
      ...
      final allRecipients = [
        ...remainingSeeders,
        ...userIds,
      ]; // Include revoked users for notification
      debugPrint(
      ...
      try {
        if (metadata != null) {
          ...
          await _socketFileClient.announceFile(
            ...
            chunkCount: metadata['chunkCount'] as int,
            availableChunks: availableChunks,
            sharedWith: remainingSeeders.isNotEmpty
                ? remainingSeeders
                : null, // ← Updated sharedWith list
          );

          debugPrint(
            '[FILE TRANSFER] ✓ File re-announced with ${remainingSeeders.length} users in sharedWith',
          );
        }
      } catch (e) {
        debugPrint('[FILE TRANSFER] Warning: Could not re-announce file: $e');
      ...
      debugPrint(
        '[FILE TRANSFER] ✓ Revoked ${userIds.length} users from share: $fileId',
      );
    } catch (e) {
  ...
  Future<String?> _getCurrentUserId() async {
    ...
    throw UnimplementedError(
      '_getCurrentUserId() needs to be implemented with your auth service',
    );
  }
  ...
  Future<List<String>?> getServerSharedWith(String fileId) async {
    try {
      ...
      debugPrint(
        '[FILE TRANSFER] Warning: Unexpected sharedWith type: ${sharedWith.runtimeType}',
      );
      return null;
    } catch (e) {
  ...
  Future<bool> _verifyFileChecksum(String fileId) async {
    try {
      ...
      if (isValid) {
        ...
        debugPrint(
          '[SECURITY]    Expected: ${expectedChecksum.substring(0, 16)}...',
        );
        debugPrint(
          '[SECURITY]    Actual:   ${actualChecksum.substring(0, 16)}...',
        );
      } else {
  ...
  Future<bool> verifyChecksumBeforeDownload(
    ...
    try {
      ...
      if (isValid) {
        ...
        debugPrint(
          '[SECURITY]    Server:   ${serverChecksum.substring(0, 16)}...',
        );
      } else {
        ...
        debugPrint(
          '[SECURITY]    ⚠️ File may be compromised - download blocked!',
        );
      }
  ...
  Future<bool> _verifyChunkHash(String fileId, int chunkIndex) async {
    try {
      ...
      if (actualHash != expectedHash) {
        ...
        debugPrint(
          '[FILE TRANSFER]    Expected: ${expectedHash.substring(0, 16)}...',
        );
        debugPrint(
          '[FILE TRANSFER]    Actual:   ${actualHash.substring(0, 16)}...',
        );
        return false;
      }
  ...
  Future<List<int>> _findCorruptedChunks(String fileId) async {
    try {
      ...
      debugPrint(
        '[FILE TRANSFER] Found ${corruptedChunks.length} corrupted chunks: $corruptedChunks',
      );
  ...
  Future<void> _recoverCorruptedFile(
    String fileId,
    List<int> corruptedChunks,
  ) async {
    try {
      debugPrint('[FILE TRANSFER] Starting smart recovery for $fileId');
      debugPrint(
        '[FILE TRANSFER] Re-downloading ${corruptedChunks.length} corrupted chunks...',
      );
      ...
      if (metadata != null) {
        ...
        final updatedChunks = downloadedChunks
            .where((c) => !corruptedChunks.contains(c))
            .toList();
