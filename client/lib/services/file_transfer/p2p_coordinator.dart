...
import 'package:universal_html/html.dart'
    as html
    show AnchorElement, Blob, Url, document;
...
class P2PCoordinator extends ChangeNotifier {
  ...
  Future<void> startDownload({
    ...
    for (final entry in seederChunks.entries) {
      ...
      debugPrint(
        '[P2P] Registered seeder: ${seederInfo.userId} -> device ${seederInfo.deviceId}',
      );
    }
  ...
  Future<void> startDownloadWithKeyRequest({
    ...
    debugPrint(
      '[P2P DOWNLOADER] Requesting encryption key from: $firstSeeder (device: ${firstSeederInfo.deviceId})',
    );
    debugPrint(
      '[P2P DOWNLOADER] ================================================',
    );
    ...
    debugPrint(
      '[P2P DOWNLOADER] ✓ PHASE 1 COMPLETE: Key received (${fileKey.length} bytes)',
    );
    debugPrint(
      '[P2P DOWNLOADER] ================================================',
    );
    ...
    debugPrint(
      '[P2P DOWNLOADER] Initializing download manager with encryption key...',
    );
    debugPrint(
      '[P2P DOWNLOADER] ================================================',
    );
    ...
    debugPrint(
      '[P2P DOWNLOADER] ✓ WebRTC connections will now transfer chunks',
    );
    debugPrint(
      '[P2P DOWNLOADER] ✓ Chunks will be decrypted with the received key',
    );
    debugPrint(
      '[P2P DOWNLOADER] ================================================',
    );
  ...
  Future<Uint8List> requestFileKey(String fileId, String peerId) async {
    ...
    if (_keyRequests.containsKey(fileId)) {
      debugPrint(
        '[P2P] WARNING: Already have a pending key request for $fileId, waiting for existing one',
      );
  ...
  void updateSeederAvailability(
    String fileId,
    Map<String, SeederInfo> seederChunks,
  ) {
    debugPrint(
      '[P2P] Updating seeder availability for $fileId: ${seederChunks.length} seeders',
    );
  ...
  Future<void> _handleKeyRequest(Map<String, dynamic> item) async {
    try {
      ...
      if (key == null) {
        ...
        debugPrint(
          '[P2P SEEDER] This file might not be shared or not fully downloaded',
        );
      ...
      if (key.length != 32) {
        debugPrint(
          '[P2P SEEDER] ⚠️  ERROR: Stored key is ${key.length} bytes, expected 32 bytes!',
        );
        debugPrint(
          '[P2P SEEDER] ⚠️  This file was uploaded with wrong key size',
        );
      ...
      debugPrint(
        '[P2P SEEDER] Sending key to $sender via Signal Protocol (E2E encrypted)...',
      );
      ...
      debugPrint(
        '[P2P SEEDER] ✓ Base64 string length: ${keyBase64.length} chars',
      );
  ...
  void _handleKeyResponse(Map<String, dynamic> item) {
    try {
      ...
      debugPrint(
        '[P2P] Current pending requests: ${_keyRequests.keys.toList()}',
      );
      ...
      if (completer == null) {
        debugPrint(
          '[P2P] WARNING: Received key response but no pending request for $fileId',
        );
        debugPrint(
          '[P2P] This might mean the request timed out or was cancelled',
        );
        return;
      }
      ...
      if (completer.isCompleted) {
        debugPrint(
          '[P2P] WARNING: Received key response but completer already completed for $fileId',
        );
        return;
      }
      ...
      if (response['error'] != null) {
        ...
        debugPrint(
          '[P2P] ⚠️  This is normal if multiple seeders exist - waiting for other seeders...',
        );
      ...
      debugPrint(
        '[P2P] ✓ Key bytes (first 8): ${key.sublist(0, min(8, key.length)).map((b) => b.toRadixString(16).padLeft(2, '0')).join(' ')}',
      );

      // Validate key length (must be 32 bytes for AES-256)
      if (key.length != 32) {
        throw Exception(
          'Invalid key length: ${key.length} bytes (expected 32 for AES-256)',
        );
      }
      ...
      try {
        final fileId = item['message'] != null
            ? (jsonDecode(item['message'])['fileId'] as String?)
            : null;
        if (fileId != null) {
          final completer = _keyRequests[fileId];
  ...
  Future<void> _handleWebRTCOffer(Map<String, dynamic> data) async {
    try {
      ...
      final String fromDeviceId = data['fromDeviceId']
          .toString(); // Convert to String (might be int)
      ...
      debugPrint('[P2P SEEDER] ========================================');
      debugPrint(
        '[P2P SEEDER] Received WebRTC offer from $fromUserId:$fromDeviceId',
      );
      ...
      webrtcService.onConnected(fromUserId, (peerId) {
        debugPrint(
          '[P2P SEEDER] ✓ WebRTC connected to $peerId for file $fileId',
        );
      });
      ...
      debugPrint(
        '[P2P SEEDER] ✓ Answer created, sending back to $fromUserId:$fromDeviceId',
      );
  ...
  Future<void> _handleICECandidate(Map<String, dynamic> data) async {
    try {
      ...
      debugPrint(
        '[P2P] Received ICE candidate from $fromUserId:$fromDeviceId for file $fileId',
      );
  ...
  void _sendICECandidate(String peerId, RTCIceCandidate candidate) {
    try {
      ...
      if (fileId == null) {
        debugPrint(
          '[P2P] WARNING: Cannot send ICE candidate - no fileId for peer $peerId',
        );
        return;
      }
      ...
      if (deviceId == null) {
        debugPrint(
          '[P2P] WARNING: Cannot send ICE candidate - no deviceId for peer $peerId',
        );
        return;
      }
  ...
  Future<void> _handleDataChannelMessage(
    ...
    try {
      debugPrint(
        '[P2P DataChannel] Received message from $peerId, type: ${data.runtimeType}',
      );

      if (data is Uint8List) {
        // Binary data - encrypted chunk
        debugPrint(
          '[P2P] Received encrypted chunk from $peerId (${data.length} bytes)',
        );
        ...
        if (type == 'requestBatchMetadata') {
          ...
          debugPrint(
            '[P2P DataChannel] Routing to _handleBatchMetadataRequest',
          );
        ...
        debugPrint(
          '[P2P DataChannel] WARNING: Unexpected data type: ${data.runtimeType}',
        );
      }
    } catch (e, stackTrace) {
  ...
  Future<void> _handleBatchMetadataRequest(
    String fileId,
    String peerId,
    ...
    try {
      ...
      debugPrint(
        '[P2P SEEDER] File has $chunkCount chunks, collecting metadata...',
      );
      ...
      debugPrint(
        '[P2P SEEDER] Collected ${allMetadata.length} chunk metadata entries',
      );
      ...
      try {
        await ackCompleter.future.timeout(
          ...
          onTimeout: () {
            ...
            throw TimeoutException(
              'Batch metadata ACK not received within 10 seconds',
            );
          },
        );

        debugPrint(
          '[P2P SEEDER] ✓ Batch metadata acknowledged, ready for chunk requests',
        );
      } finally {
        // Clean up completer
  ...
  Future<void> _handleChunkRequest(
    ...
    try {
      ...
      debugPrint(
        '[P2P SEEDER] ✓ Sending chunk $chunkIndex (${encryptedChunk.length} bytes)',
      );
      ...
      if (statsProvider != null) {
        ...
        final chunkCount = await storage.getChunkCount(fileId);
        final sentChunks =
            (_seederChunksSent[peerId]?[fileId]?.length ?? 0) + 1;
  ...
  Future<void> _checkSeederTransferComplete(
    String fileId,
    String peerId,
  ) async {
    try {
      ...
      debugPrint(
        '[P2P SEEDER] Transfer progress to $peerId: $sentChunks/$chunkCount chunks',
      );
      ...
      if (sentChunks >= chunkCount) {
        ...
        debugPrint(
          '[P2P SEEDER] Starting inactivity timer (${_seederInactivityTimeout.inSeconds}s)...',
        );
        debugPrint(
          '[P2P SEEDER] ================================================',
        );
  ...
  void _startSeederCleanupTimer(String fileId, String peerId) async {
    ...
    if (lastActivity != null && lastActivity.isAfter(startTime)) {
      debugPrint(
        '[P2P SEEDER] Activity detected from $peerId, extending timer',
      );
    ...
    debugPrint('[P2P SEEDER] ================================================');
    debugPrint(
      '[P2P SEEDER] CLEANUP: Closing connection to $peerId (inactivity)',
    );
    debugPrint('[P2P SEEDER] FileId: $fileId');
    debugPrint(
      '[P2P SEEDER] No requests for ${_seederInactivityTimeout.inSeconds}s',
    );
    debugPrint('[P2P SEEDER] ================================================');
  ...
  Future<void> _buildChunkQueue(String fileId, int chunkCount) async {
    ...
    debugPrint(
      '[P2P] Built chunk queue for $fileId: ${missingChunks.length} missing chunks',
    );
  }
  ...
  Future<void> _connectToSeeders(String fileId) async {
    ...
    for (final entry in seedersToConnect) {
      try {
        ...
        debugPrint(
          '[P2P] Failed to connect to seeder ${entry.value.userId}: $e',
        );
      }
    }
  }
  ...
  Future<void> _handleIncomingChunk(
    ...
    try {
      ...
      if (phase == DownloadPhase.assembling ||
          phase == DownloadPhase.complete) {
        debugPrint(
          '[P2P] ⚠️ Ignoring late chunk from $peerId (download in phase: $phase)',
        );
        return;
      }
      ...
      debugPrint(
        '[P2P] Received chunk data from $peerId (${encryptedChunk.length} bytes)',
      );

      // ✅ FIX: Extract chunkIndex from 4-byte header
      if (encryptedChunk.length < 4) {
        debugPrint(
          '[P2P] ERROR: Chunk data too small (no header): ${encryptedChunk.length} bytes',
        );
        return;
      }
      ...
      debugPrint(
        '[P2P] ✓ Chunk identified from header: $chunkIndex (${actualEncryptedChunk.length} bytes payload)',
      );
      ...
      if (metadata == null) {
        debugPrint(
          '[P2P] ERROR: Received chunk data without metadata from $peerId',
        );
      ...
      if (downloadTask != null &&
          downloadTask.completedChunks.contains(chunkIndex)) {
        debugPrint(
          '[P2P] Ignoring duplicate chunk $chunkIndex from $peerId (already in completed set)',
        );
      ...
      if (!wasSaved) {
        debugPrint(
          '[P2P] Chunk $chunkIndex was duplicate, skipping progress update',
        );
        _activeChunkRequests[fileId]?.remove(chunkIndex);
        _completeChunkRequest(peerId, chunkIndex);
        return;
      ...
      if (task != null) {
        ...
        debugPrint(
          '[P2P] Download progress: ${task.downloadedChunks}/${task.chunkCount} chunks',
        );
  ...
  Future<void> _handleChunkError(
    ...
    debugPrint(
      '[P2P ChunkRetry] Attempt: ${retryInfo.attemptCount + 1}/$maxChunkRetries',
    );
    debugPrint(
      '[P2P ChunkRetry] Failed seeders: ${_chunkFailedSeeders[fileId]![chunkIndex]!.toList()}',
    );

    // Check if max retries exceeded
    if (retryInfo.attemptCount >= maxChunkRetries) {
      debugPrint(
        '[P2P ChunkRetry] ❌ Max retries exceeded for chunk $chunkIndex',
      );
    ...
    final alternateSeeder = _findAlternateSeeder(
      fileId,
      chunkIndex,
      failedSeeders,
    );
    ...
    if (alternateSeeder != null) {
      ...
      debugPrint(
        '[P2P ChunkRetry] Will retry after ${retryInfo.getBackoffDelay().inSeconds}s backoff',
      );
      debugPrint(
        '[P2P ChunkRetry] ================================================',
      );
      ...
      debugPrint(
        '[P2P ChunkRetry] Requesting chunk $chunkIndex from alternate seeder $alternateSeeder',
      );
      await _requestChunk(fileId, alternateSeeder, chunkIndex);
    } else {
      debugPrint('[P2P ChunkRetry] ⚠️ No alternate seeder available');
      debugPrint(
        '[P2P ChunkRetry] Will retry with same seeder after ${retryInfo.getBackoffDelay().inSeconds}s',
      );
      debugPrint(
        '[P2P ChunkRetry] ================================================',
      );
  ...
  String? _findAlternateSeeder(
    String fileId,
    int chunkIndex,
    Set<String> failedSeeders,
  ) {
    final seeders = _seederAvailability[fileId] ?? {};
  ...
  Future<void> _drainAndComplete(String fileId, dynamic task) async {
    ...
    while (DateTime.now().difference(startTime) < _drainTimeout) {
      ...
      if (remaining == 0) {
        ...
        debugPrint(
          '[P2P] ✓ All in-flight chunks received (${elapsed.inMilliseconds}ms)',
        );
        debugPrint('[P2P] ================================================');
    ...
    debugPrint(
      '[P2P] Missing chunks: ${(_activeChunkRequests[fileId] ?? {}).keys.toList()}',
    );
    debugPrint('[P2P] ================================================');
  ...
  Future<void> _handleBatchMetadata(
    String fileId,
    String peerId,
    ...
    try {
      ...
      debugPrint(
        '[P2P] ✓ Batch metadata cached for ${metadataList.length} chunks',
      );
      debugPrint('[P2P] Sending batch metadata ACK...');

      // Send ACK
      await webrtcService.sendData(peerId, {'type': 'batchMetadataAck'});
  ...
  Future<void> _handleChunkResponse(
    String fileId,
    String peerId,
    Map<String, dynamic> data,
  ) async {
    try {
      ...
      debugPrint(
        '[P2P] Received chunk metadata: chunk $chunkIndex ($size bytes) from $peerId',
      );
      if (ivString != null) {
        debugPrint(
          '[P2P] Chunk has IV for decryption (${ivString.substring(0, 10)}...)',
        );
      }
  ...
  void _handleChunkMetadataAck(String peerId, Map<String, dynamic> data) {
    ...
    debugPrint(
      '[P2P] Received legacy chunk metadata ACK (ignored with batch protocol)',
    );
  }

  void _handleChunkUnavailable(
    String fileId,
    String peerId,
    Map<String, dynamic> data,
  ) {
  ...
  void _requestNextChunks(String fileId, String peerId) {
    ...
    final activeRequests = _activeChunkRequests[fileId] ?? {};
    final currentPeerRequests = activeRequests.values
        .where((p) => p == peerId)
        .length;
    ...
    if (inFlight >= maxChunksInFlightPerPeer) {
      debugPrint(
        '[P2P RateLimit] Peer $peerId has $inFlight chunks in flight, waiting...',
      );
      return;
    }
    ...
    final availableChunks = queue
        .where((chunk) => seederChunks.contains(chunk))
        .toList();
  ...
  Future<void> _requestChunkWithRateLimit(
    String fileId,
    String peerId,
    int chunkIndex,
  ) async {
  ...
  void _sendSignalingMessage(String peerId, Map<String, dynamic> message) {
    ...
    if (targetDeviceId == null) {
      debugPrint(
        '[P2P] WARNING: No deviceId for peer $peerId, cannot send signaling message',
      );
    ...
    switch (type) {
      case 'offer':
        ...
        socketClient.sendWebRTCOffer(
          targetUserId: peerId,
          targetDeviceId:
              targetDeviceId ?? '', // Empty string triggers broadcast on server
          fileId: fileId,
          offer: offer,
        );
        debugPrint(
          '[P2P] ✓ WebRTC offer sent to $peerId${targetDeviceId != null ? ':$targetDeviceId' : ' (broadcast)'}',
        );
        break;
      ...
      case 'answer':
        ...
        debugPrint(
          '[P2P] ✓ WebRTC answer sent to $peerId${targetDeviceId != null ? ':$targetDeviceId' : ' (broadcast)'}',
        );
        break;
    ...
    try {
      ...
      for (int i = 0; i < task.chunkCount; i++) {
        ...
        if (ivData == null) {
          ...
          throw Exception(
            'Invalid IV type for chunk $i: ${ivData.runtimeType}',
          );
        }
...
class AdaptiveThrottler {
  ...
  void recordSuccess() {
    ...
    if (_successCount >= successThreshold) {
      ...
      if (canSpeedup && _currentMaxInFlight < maxLimit) {
        ...
        _successCount = 0;
        debugPrint(
          '[AdaptiveThrottle] ↑ Increased limit to $_currentMaxInFlight',
        );
      }
    }
  }
  ...
  void recordBackpressure(int bufferedAmount) {
    ...
    if (canSlowdown && bufferedAmount > 10 * 1024 * 1024) {
      // > 10 MB
      if (_currentMaxInFlight > minLimit) {
        ...
        debugPrint(
          '[AdaptiveThrottle] ↓ Decreased limit to $_currentMaxInFlight (buffer: ${(bufferedAmount / 1024 / 1024).toStringAsFixed(1)} MB)',
        );
      }
    }
  }
...
class ChunkRetryInfo {
  ...
  ChunkRetryInfo({this.attemptCount = 0, this.lastAttempt, this.lastSeeder});
