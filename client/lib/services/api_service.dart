...
class UnauthorizedInterceptor extends Interceptor {
  ...
  void onResponse(Response response, ResponseInterceptorHandler handler) {
    ...
    if (response.statusCode == 401) {
      ...
      debugPrint(
        '[API] 401 detected - isLoggedIn: ${AuthService.isLoggedIn}, isWeb: $kIsWeb',
      );
      if (AuthService.isLoggedIn || !kIsWeb) {
        debugPrint(
          '[API] ⚠️  401 Unauthorized detected - triggering auto-logout',
        );
        debugPrint(
          '[API] Callback exists: ${_globalUnauthorizedCallback != null}',
        );
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) {
    ...
    if (err.response?.statusCode == 401) {
      ...
      // For web: Only trigger if already logged in
      debugPrint(
        '[API] 401 error detected - isLoggedIn: ${AuthService.isLoggedIn}, isWeb: $kIsWeb',
      );
      if (AuthService.isLoggedIn || !kIsWeb) {
        debugPrint(
          '[API] ⚠️  401 Unauthorized detected in error - triggering auto-logout',
        );
        debugPrint(
          '[API] Callback exists: ${_globalUnauthorizedCallback != null}',
        );
        ...
        debugPrint(
          '[API] 401 Unauthorized error - user not logged in yet, ignoring',
        );
      }
    }
...
class RetryInterceptor extends Interceptor {
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) async {
    ...
    if (_shouldRetry(err) && retryCount < maxRetries) {
      debugPrint(
        '[API RETRY] Attempt ${retryCount + 1}/$maxRetries for ${err.requestOptions.path}',
      );
