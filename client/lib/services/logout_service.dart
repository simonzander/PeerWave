...
class LogoutService {
  ...
  Future<void> logout(
    ...
    try {
      debugPrint('[LOGOUT] ========================================');
      debugPrint(
        '[LOGOUT] Starting logout process... (userInitiated: $userInitiated)',
      );
      debugPrint('[LOGOUT] ========================================');
      ...
      if (!kIsWeb && !userInitiated) {
        try {
          debugPrint(
            '[LOGOUT] Native: Checking authentication status before auto-logout...',
          );
          ...
          if (hasSession) {
            ...
            if (kIsWeb) {
              ...
              if (activeServer == null) {
                debugPrint(
                  '[LOGOUT] No active server configured, proceeding with logout',
                );
            ...
            if (urlString == null || urlString.isEmpty) {
              debugPrint(
                '[LOGOUT] No server URL available, proceeding with logout',
              );
              ...
              debugPrint(
                '[LOGOUT] Auth check response: ${response.statusCode}',
              );
              ...
              if (response.statusCode == 200 && response.data != null) {
                ...
                if (isAuthenticated) {
                  debugPrint(
                    '[LOGOUT] ✓ Session is still valid - preventing auto-logout',
                  );
                  ...
                  debugPrint(
                    '[LOGOUT] Session invalid (${authData['reason']}), proceeding with logout',
                  );
                }
              } else {
                debugPrint(
                  '[LOGOUT] Auth check failed (status: ${response.statusCode}), proceeding with logout',
                );
              }
            }
          } else {
      ...
      if (kIsWeb) {
        ...
        try {
          ...
          if (kIsWeb) {
            try {
              ...
              debugPrint(
                '[LOGOUT] ✓ All encryption keys cleared from SessionStorage',
              );
            } catch (e) {
            ...
            try {
              ...
              if (localStorage.containsKey('clientid')) {
                ...
                debugPrint(
                  '[LOGOUT] ✓ Removed deprecated localStorage key: clientid',
                );
              }
        ...
        try {
          ...
          if (activeServer != null) {
            ...
            debugPrint(
              '[LOGOUT] ✓ Device identity cleared for server: ${activeServer.serverUrl}',
            );
          } else {
            await DeviceIdentityService.instance.clearDeviceIdentity();
      ...
      try {
        ...
        debugPrint(
          '[LOGOUT] ⚠ Server logout failed (may already be logged out): $e',
        );
      }
      ...
      if (showMessage && validContext != null && validContext.mounted) {
        try {
          ...
          debugPrint(
            '[LOGOUT] ⚠ Could not show logout message (ScaffoldMessenger not available): $e',
          );
        }
      }
      ...
      if (validContext != null && validContext.mounted) {
        try {
          ...
          try {
            ...
            debugPrint(
              '[LOGOUT] ⚠ GoRouter not in context (expected during initialization)',
            );
            ...
            if (kIsWeb) {
              ...
              debugPrint(
                '[LOGOUT] Waiting for redirect handler to catch navigation',
              );
            }
          }
        } catch (e) {
  ...
  Future<void> autoLogoutWithNavigatorKey(
    GlobalKey<NavigatorState> navigatorKey,
  ) async {
    debugPrint(
      '[LOGOUT] 🔴 Auto-logout triggered - 401 Unauthorized (using navigator key)',
    );
  ...
  Future<void> autoLogout(BuildContext? context) async {
    ...
    if (validContext != null && validContext.mounted) {
      ...
      debugPrint(
        '[LOGOUT] No valid context - session expired, redirect handler will navigate',
      );
    }
  }
}
