...
class MessageListenerService {
  ...
  void registerVideoConferenceService(VideoConferenceService service) {
    ...
    debugPrint(
      '[MESSAGE_LISTENER] VideoConferenceService registered for E2EE key handling',
    );
  }
  ...
  void _triggerNotification(MessageNotification notification) {
    debugPrint(
      '[MESSAGE_LISTENER] Triggering notification: ${notification.type} from ${notification.senderId}',
    );
  ...
  Future<void> _handleDirectMessage(dynamic data) async {
    try {
      ...
      debugPrint(
        '[MESSAGE_LISTENER] 1:1 message notification triggered: $itemId',
      );
    } catch (e) {
  ...
  Future<void> _handleGroupMessage(dynamic data) async {
    try {
      ...
      if (itemId == null ||
          ...
          payload == null) {
        debugPrint(
          '[MESSAGE_LISTENER] Missing required fields in group message',
        );
        return;
      }
      ...
      try {
        ...
        if (itemType == 'video_key_request' ||
            itemType == 'video_key_response') {
        ...
        try {
          ...
          if (!profileService.isProfileCached(senderId)) {
            debugPrint(
              "[MESSAGE_LISTENER] Loading profile for group message sender: $senderId",
            );
          ...
          debugPrint(
            "[MESSAGE_LISTENER] ⚠ Failed to load sender profile (server may be unavailable): $e",
          );
        ...
        debugPrint(
          '[MESSAGE_LISTENER] Group message decrypted and stored: $itemId',
        );
      } catch (e) {
  ...
  Future<void> _processFileShareUpdate({
    ...
    try {
      ...
      final affectedUserIds =
          (shareData['affectedUserIds'] as List?)?.cast<String>() ?? [];
      final checksum = shareData['checksum'] as String?;

      if (fileId == null || action == null) {
        debugPrint(
          '[MESSAGE_LISTENER] Missing fileId or action in share update',
        );
        return;
      }

      debugPrint(
        '[MESSAGE_LISTENER] File share update: $action for file $fileId',
      );
      debugPrint('[MESSAGE_LISTENER] Affected users: $affectedUserIds');
      if (checksum != null) {
        debugPrint(
          '[MESSAGE_LISTENER] Checksum: ${checksum.substring(0, 16)}...',
        );
      }
      ...
      debugPrint(
        '[P2P] Processing Signal-authenticated share update (action: $action)',
      );
      ...
      if (action == 'add') {
        ...
        if (fileTransferService != null && socketFileClient != null) {
          try {
            ...
            if (metadata != null) {
              ...
              final existingSharedWith = List<String>.from(
                metadata['sharedWith'] ?? [],
              );
              ...
              if (mergedSharedWith.length != existingSharedWith.length) {
                ...
                debugPrint(
                  '[FILE SHARE] ✓ Merged sender into sharedWith: ${mergedSharedWith.length} users',
                );
                debugPrint('[FILE SHARE] sharedWith: $mergedSharedWith');
              } else {
                debugPrint(
                  '[FILE SHARE] Sender already in sharedWith - no change needed',
                );
              }
            } else {
              // File doesn't exist yet - save minimal metadata with sender in sharedWith
              debugPrint(
                '[FILE SHARE] File not downloaded yet - saving metadata for future download',
              );
              ...
              try {
                ...
                debugPrint(
                  '[FILE SHARE] ✓ Saved file metadata with sender in sharedWith',
                );
              } catch (e) {
                debugPrint(
                  '[FILE SHARE] Warning: Could not save file metadata: $e',
                );
              }
            }
          } catch (e) {
            debugPrint(
              '[FILE SHARE] Warning: Could not update local sharedWith: $e',
            );
          }
        }
        ...
        if (fileTransferService != null) {
          try {
            debugPrint(
              '[FILE SHARE] Canceling active downloads for revoked file...',
            );
          ...
          try {
            debugPrint(
              '[FILE SHARE] Deleting downloaded chunks for revoked file...',
            );
  ...
  Future<void> _processVideoE2EEKey({
    ...
    try {
      ...
      if (messageType == 'video_e2ee_key_request') {
        ...
        debugPrint(
          '[MESSAGE_LISTENER][TEST] Request Timestamp: $requestTimestamp',
        );
        debugPrint(
          '[MESSAGE_LISTENER][TEST] 🔍 Checking VideoConferenceService...',
        );
        debugPrint(
          '[MESSAGE_LISTENER][TEST]   - Service registered: ${_videoConferenceService != null}',
        );
        debugPrint(
          '[MESSAGE_LISTENER][TEST]   - Has E2EE key: ${_videoConferenceService?.hasE2EEKey ?? false}',
        );
        ...
        if (requesterId == currentUserId && senderDeviceId == currentDeviceId) {
          debugPrint(
            '[MESSAGE_LISTENER][TEST] ℹ️ Ignoring own key request from same device (true echo)',
          );
          debugPrint(
            '═══════════════════════════════════════════════════════════',
          );
        ...
        debugPrint(
          '[MESSAGE_LISTENER][TEST] ℹ️ Request from different device - will respond',
        );
        debugPrint(
          '[MESSAGE_LISTENER][TEST]   - Requester device: $senderDeviceId',
        );
        debugPrint(
          '[MESSAGE_LISTENER][TEST]   - Current device: $currentDeviceId',
        );

        // Respond if we have a key available (even if not connected to LiveKit room yet)
        if (_videoConferenceService != null &&
            _videoConferenceService!.hasE2EEKey) {
          debugPrint(
            '[MESSAGE_LISTENER][TEST] ✓ Forwarding to VideoConferenceService.handleKeyRequest()',
          );
          ...
          debugPrint(
            '[MESSAGE_LISTENER][TEST] ⚠️ VideoConferenceService not available or no key generated',
          );
          debugPrint(
            '[MESSAGE_LISTENER][TEST] ℹ️ This is expected if you are the requester waiting for response',
          );
        }
      ...
      if (messageType == 'video_e2ee_key_response') {
        ...
        debugPrint(
          '[MESSAGE_LISTENER][TEST] Key Length: ${encryptedKey?.length ?? 0} chars (base64)',
        );
        ...
        if (encryptedKey != null &&
            ...
            _videoConferenceService != null) {
          debugPrint(
            '[MESSAGE_LISTENER][TEST] ✓ Forwarding to VideoConferenceService.handleE2EEKey()',
          );
          await _videoConferenceService!.handleE2EEKey(
            ...
            channelId: channelId,
            timestamp:
                keyTimestamp, // Pass timestamp for race condition resolution
          );
        } else {
          debugPrint(
            '[MESSAGE_LISTENER][TEST] ⚠️ Missing data or VideoConferenceService not available',
          );
        }
      ...
      if (type == 'video_key_request') {
        ...
        final requesterId = keyData['requesterId'] as String?;
        debugPrint(
          '[MESSAGE_LISTENER][TEST] [LEGACY] Received key request from $requesterId',
        );

        if (_videoConferenceService != null &&
            _videoConferenceService!.isConnected) {
          await _videoConferenceService!.handleKeyRequest(
            requesterId ?? senderId,
          );
        }
      ...
      if (type == 'video_key_response') {
        ...
        debugPrint(
          '[MESSAGE_LISTENER] [LEGACY] Received key response for user: $targetUserId (no timestamp, using current: $legacyTimestamp)',
        );
      ...
      debugPrint(
        '[MESSAGE_LISTENER] Unknown video E2EE key type: $type or messageType: $messageType',
      );
    } catch (e) {
  ...
  Future<void> _handleFileShareUpdate(dynamic data) async {
    debugPrint(
      '[MESSAGE_LISTENER] ⚠️ Received file_share_update via dedicated event (deprecated)',
    );
    debugPrint(
      '[MESSAGE_LISTENER] File shares should arrive as groupItem with type=file_share_update',
    );
  ...
  void _handleGroupDeliveryReceipt(dynamic data) {
    try {
      ...
      debugPrint(
        '[MESSAGE_LISTENER] Error handling group delivery receipt: $e',
      );
    }
  }
