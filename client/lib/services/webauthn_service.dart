...
class WebAuthnService {
  ...
  Future<void> captureWebAuthnResponse(String credentialId, String signatureBase64) async {
    try {
      ...
      debugPrint('[WEBAUTHN_SERVICE] Credential ID: ${credentialId.substring(0, 16)}...');
      debugPrint('[WEBAUTHN_SERVICE] Signature length: ${_lastSignature!.length} bytes');
    } catch (e) {
  ...
  Future<void> initializeDeviceEncryption(String email, String clientId) async {
    if (_currentCredentialId == null || _lastSignature == null) {
      throw Exception('No WebAuthn response captured. Call captureWebAuthnResponse first.');
    }
    ...
    final encryptionKey = await WebAuthnCryptoService.instance.deriveEncryptionKey(credentialBytes);
    debugPrint('[WEBAUTHN_SERVICE] ✓ Encryption key derived from credentialId (${encryptionKey.length} bytes)');
  ...
  bool get hasWebAuthnResponse => _currentCredentialId != null && _lastSignature != null;
}
