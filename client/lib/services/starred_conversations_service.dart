...
class StarredConversationsService {
  static final StarredConversationsService _instance =
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _starredConversations = results
          .map((row) => row['user_uuid'] as String)
          .toSet();
      _initialized = true;

      debugPrint(
        '[STARRED_CONV] Initialized with ${_starredConversations.length} starred conversations',
      );
    } catch (e) {
  ...
  bool isStarred(String userUuid) {
    if (!_initialized) {
      debugPrint(
        '[STARRED_CONV] Warning: Service not initialized, returning false',
      );
      return false;
    }
  ...
  Future<bool> starConversation(String userUuid) async {
    try {
      ...
      final db = await DatabaseHelper.database;
      await db.insert('starred_conversations', {
        'user_uuid': userUuid,
  ...
  List<String> getStarredConversations() {
    if (!_initialized) {
      debugPrint(
        '[STARRED_CONV] Warning: Service not initialized, returning empty list',
      );
      return [];
    }
