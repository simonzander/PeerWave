...
(function dartProgram() {
  ...
  A = {JS_CONST: function JS_CONST() {
    ...
    FrameCryptor: function FrameCryptor(t0, t1, t2, t3, t4, t5, t6) {
      ...
      _._transformIsActive = _._enabled = false;
      _.lastError = t4;
      _.currentKeyIndex = 0;
      _.worker = t5;
      _.sifGuard = t6;
    ...
    isH265SliceNALU(naluType) {
      return naluType === 0 || naluType === 1 || naluType === 2 || naluType === 3 || naluType === 4 || naluType === 5 || naluType === 6 || naluType === 7 || naluType === 8 || naluType === 9 || naluType === 16 || naluType === 17 || naluType === 18 || naluType === 19 || naluType === 20 || naluType === 21;
    },
    findSliceNALUUnencryptedBytes(data, naluIndices, codec) {
      var t1, t2, t3, _i, index, type;
      for (t1 = naluIndices.length, t2 = data.length, t3 = codec === "h265", _i = 0; _i < t1; ++_i) {
        index = naluIndices[_i];
        if (t3) {
          if (!(index < t2))
            return A.ioore(data, index);
          if (A.isH265SliceNALU(data[index] >>> 1 & 63))
            return index + 2;
        } else {
          if (!(index < t2))
            return A.ioore(data, index);
    ...
    findNALUIndices(stream) {
      var t1, start, pos0, t2, end,
        result = A._setArrayType([], type$.JSArray_int),
        pos = stream.length,
        searchLength = pos - 3;
      for (t1 = searchLength - 1, start = 0, pos0 = 0; pos0 < searchLength; start = pos0) {
        for (; pos0 < searchLength;) {
          if (pos0 < t1) {
            if (!(pos0 >= 0))
              return A.ioore(stream, pos0);
            t2 = stream[pos0] === 0 && stream[pos0 + 1] === 0 && stream[pos0 + 2] === 0 && stream[pos0 + 3] === 1;
          } else
            t2 = false;
          if (t2)
            break;
          if (!(pos0 >= 0))
            return A.ioore(stream, pos0);
          if (stream[pos0] === 0 && stream[pos0 + 1] === 0 && stream[pos0 + 2] === 1)
            break;
          ++pos0;
        }
    ...
    processNALUsForEncryption(data, knownCodec) {
      var unencryptedBytes,
        naluIndices = A.findNALUIndices(data);
      if (knownCodec === "unknown")
        return new A.NALUProcessingResult(0, knownCodec);
      unencryptedBytes = A.findSliceNALUUnencryptedBytes(data, naluIndices, knownCodec);
      if (unencryptedBytes == null)
        throw A.wrapException(A.Exception_Exception("Could not find NALU"));
      return new A.NALUProcessingResult(unencryptedBytes, knownCodec);
    },
    NALUProcessingResult: function NALUProcessingResult(t0, t1) {
      this.unencryptedBytes = t0;
      this.detectedCodec = t1;
    },
  ...
  A.E2EEDataPacketCryptor.prototype = {
    ...
    encrypt$body$E2EEDataPacketCryptor(keys, data) {
      var $async$goto = 0,
        ...
        $async$returnValue, $async$handler = 2, $async$errorStack = [], $async$self = this, secretKey, keyIndex, iv, cipherText, e, t3, iv0, t4, frameTrailer, t5, t6, exception, t1, t2, $async$exception, $async$temp1;
        ...
        while (true)
          switch ($async$goto) {
            case 0:
              ...
              $async$handler = 4;
              t3 = type$.JSObject;
              t4 = t3._as(t3._as($async$self.worker.crypto).subtle);
              t5 = type$.Object;
              t6 = A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["name", "AES-GCM", "iv", iv], type$.String, t5));
              t5 = t6 == null ? t5._as(t6) : t6;
              $async$temp1 = type$.NativeByteBuffer;
              $async$goto = 7;
              return A._asyncAwait(A.promiseToFuture(t3._as(t4.encrypt(t5, secretKey, data)), type$.nullable_Object), $async$encrypt$2);
            case 7:
      ...
      var $async$decrypt$2 = A._wrapJsFunctionForAsync(function($async$errorCode, $async$result) {
        ...
        while (true)
          switch ($async$goto) {
            ...
            case 10:
              // after finally
              if (t1.decrypted == null) {
                t1 = A.Exception_Exception(string$.x5bdecod);
                throw A.wrapException(t1);
              }
              t3 = $async$self.keyHandler;
              t3._decryptionFailureCount = 0;
              t3._hasValidKey = true;
              t3 = J.get$length$asx(payload);
              t4 = t1.decrypted;
              t4.toString;
              t2.log$4(B.Level_FINER_400, string$.decodex20 + t3 + ", decrypted: " + A.NativeUint8List_NativeUint8List$view(t4, 0, null).length, null, null);
              t1 = t1.decrypted;
              t1.toString;
  ...
  A.E2EEDataPacketCryptor_decrypt_decryptFrameInternal.prototype = {
    call$0() {
      var $async$goto = 0,
        $async$completer = A._makeAsyncAwaitCompleter(type$.void),
        $async$self = this, decrypted, t1, t2, t3, t4, t5, $async$temp1;
        ...
        while (true)
          switch ($async$goto) {
            case 0:
              ...
              t1 = $async$self.$this;
              t2 = type$.JSObject;
              t3 = t2._as(t2._as(t1.worker.crypto).subtle);
              t4 = type$.Object;
              t5 = A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["name", "AES-GCM", "iv", $async$self.iv], type$.String, t4));
              t4 = t5 == null ? t4._as(t5) : t5;
              t5 = $async$self._box_0;
              $async$temp1 = type$.NativeByteBuffer;
              $async$goto = 2;
              return A._asyncAwait(A.promiseToFuture(t2._as(t3.decrypt(t4, t5.currentkeySet.encryptionKey, $async$self.payload)), type$.nullable_Object), $async$call$0);
            case 2:
              // returning from await.
              decrypted = $async$temp1._as($async$result);
              t4 = $async$self._box_1;
              t4.decrypted = decrypted;
              t3 = $.$get$logger();
              t3.log$4(B.Level_FINER_400, string$.decodex3ad + A.NativeUint8List_NativeUint8List$view(decrypted, 0, null).length, null, null);
              t2 = t4.decrypted;
              if (t2 == null)
                throw A.wrapException(A.Exception_Exception("[decryptFrameInternal] could not decrypt"));
              t3.log$4(B.Level_FINER_400, string$.decodex3ad + A.NativeUint8List_NativeUint8List$view(t2, 0, null).length, null, null);
              $async$goto = t5.currentkeySet !== t4.initialKeySet ? 3 : 4;
              break;
            case 3:
              // then
              t3.log$4(B.Level_FINE_500, string$.decodex3ar, null, null);
              $async$goto = 5;
              return A._asyncAwait(t1.keyHandler.setKeySetFromMaterial$2(t5.currentkeySet, $async$self.initialKeyIndex), $async$call$0);
            case 5:
              // returning from await.
            case 4:
              // join
  ...
  A.FrameCryptor.prototype = {
    ...
    setupTransform$7$codec$isReuse$kind$operation$readable$trackId$writable(codec, isReuse, kind, operation, readable, trackId, writable) {
      return this.setupTransform$body$FrameCryptor(codec, isReuse, kind, operation, readable, trackId, writable);
    },
    setupTransform$6$isReuse$kind$operation$readable$trackId$writable(isReuse, kind, operation, readable, trackId, writable) {
      return this.setupTransform$7$codec$isReuse$kind$operation$readable$trackId$writable(null, isReuse, kind, operation, readable, trackId, writable);
    },
    setupTransform$body$FrameCryptor(codec, isReuse, kind, operation, readable, trackId, writable) {
      var $async$goto = 0,
        $async$completer = A._makeAsyncAwaitCompleter(type$.void),
        $async$returnValue, $async$self = this, transformer, e, t2, t3, t4, exception, t1;
      var $async$setupTransform$7$codec$isReuse$kind$operation$readable$trackId$writable = A._wrapJsFunctionForAsync(function($async$errorCode, $async$result) {
        if ($async$errorCode === 1)
          return A._asyncRethrow($async$result, $async$completer);
        while (true)
          switch ($async$goto) {
            case 0:
              ...
              if (isReuse && $async$self._transformIsActive) {
                t1.log$4(B.Level_INFO_800, "setupTransform: transform already active, skipping setup", null, null);
                // goto return
                $async$goto = 1;
                break;
              }
              t1 = self.TransformStream;
              t2 = operation === "encode" ? A._functionToJS2($async$self.get$encodeFunction()) : A._functionToJS2($async$self.get$decodeFunction());
              t3 = type$.String;
              t4 = type$.JSObject;
              transformer = t4._as(new t1(t4._as(A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["transform", t2], t3, type$.JavaScriptFunction)))));
              try {
                t4._as(t4._as(readable.pipeThrough(transformer)).pipeTo(writable));
              } catch (exception) {
                e = A.unwrapException(exception);
                ...
                if ($async$self.lastError !== B.CryptorError_7) {
                  ...
                  $async$self.worker.postMessage(A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["type", "cryptorState", "msgType", "event", "participantId", $async$self.participantIdentity, "state", "internalError", "error", "Internal error: " + J.toString$0$(e)], t3, type$.nullable_String)));
                }
              }
              $async$self._transformIsActive = true;
      ...
      return A._asyncStartSync($async$setupTransform$7$codec$isReuse$kind$operation$readable$trackId$writable, $async$completer);
    },
    getUnencryptedBytes$2(obj, codec) {
      var data, t1, t2, result, t3, t4, _this = this, _null = null,
        _s28_ = "Unsupported codec for track ",
        frameType = "";
      if (A.JSAnyUtilityExtension_instanceOfString(obj, "RTCEncodedVideoFrame")) {
        ...
        data = _null;
      t1 = A._setArrayType(["h264", "h265"], type$.JSArray_String);
      t2 = codec == null ? _null : codec.toLowerCase();
      if (B.JSArray_methods.contains$1(t1, t2 == null ? "" : t2)) {
        if (data == null)
          throw A.wrapException(A.StateError$("Frame data is null for codec " + A.S(codec)));
        codec.toString;
        result = A.processNALUsForEncryption(data, codec);
        t1 = result.detectedCodec;
        if (t1 === "unknown") {
          if (_this.lastError !== B.CryptorError_4) {
            _this.lastError = B.CryptorError_4;
            t2 = _this.participantIdentity;
            t3 = _this.trackId;
            t4 = _this.__FrameCryptor_kind_A;
            t4 === $ && A.throwLateFieldNI("kind");
            _this.worker.postMessage(A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["type", "cryptorState", "msgType", "event", "participantId", t2, "trackId", t3, "kind", t4, "state", "unsupportedCodec", "error", _s28_ + t3 + ", detected codec " + t1], type$.String, type$.nullable_String)));
          }
          throw A.wrapException(A.Exception_Exception(_s28_ + _this.trackId));
        }
    ...
    readFrameInfo$1(frameObj) {
      var buffer, frameType, meta, synchronizationSource, timestamp, _null = null;
      new Uint8Array(0);
      if (A.JSAnyUtilityExtension_instanceOfString(frameObj, "RTCEncodedVideoFrame")) {
        buffer = A.NativeUint8List_NativeUint8List$view(type$.NativeByteBuffer._as(frameObj.data), 0, _null);
        if ("type" in frameObj) {
          frameType = A._asString(frameObj.type);
          $.$get$logger().log$4(B.Level_FINER_400, "frameType: " + frameType, _null, _null);
        } else
          frameType = "";
        meta = type$.JSObject._as(frameObj.getMetadata());
        synchronizationSource = A._asInt(meta.synchronizationSource);
        if ("rtpTimestamp" in meta)
          timestamp = B.JSInt_methods.toInt$0(A._asInt(meta.rtpTimestamp));
        else
          timestamp = "timestamp" in frameObj ? A._asInt(A._asDouble(frameObj.timestamp)) : 0;
      } else {
        if (A.JSAnyUtilityExtension_instanceOfString(frameObj, "RTCEncodedAudioFrame")) {
          buffer = A.NativeUint8List_NativeUint8List$view(type$.NativeByteBuffer._as(frameObj.data), 0, _null);
          meta = type$.JSObject._as(frameObj.getMetadata());
          synchronizationSource = A._asInt(meta.synchronizationSource);
          if ("rtpTimestamp" in meta)
          ...
          throw A.wrapException(A.Exception_Exception("encodeFunction: frame is not a RTCEncodedVideoFrame or RTCEncodedAudioFrame"));
        frameType = "audio";
      }
      return new A.FrameInfo(frameType, synchronizationSource, timestamp, buffer);
    },
    enqueueFrame$3(frameObj, controller, buffer) {
      frameObj.data = type$.NativeByteBuffer._as(B.NativeUint8List_methods.get$buffer(buffer.toBytes$0()));
          ...
          switch ($async$goto) {
            case 0:
              ...
              if (t3 != null) {
                ...
                t6 = t5 + 1;
                if (t4.length > t6) {
                  magicBytesBuffer = B.NativeUint8List_methods.sublist$2(srcFrame.buffer, srcFrame.buffer.length - t5, srcFrame.buffer.length);
                  ...
                  if (A.Iterable_iterableToFullString(magicBytesBuffer, "[", "]") === A.Iterable_iterableToFullString(t3, "[", "]")) {
                    ...
                    if (t1) {
                      ...
                      t2.log$4(B.Level_FINER_400, "encodeFunction: skip unencrypted frame, type " + t1[0], null, null);
                      ...
                      t2.log$4(B.Level_FINE_500, "encodeFunction: enqueing silent frame", null, null);
                      $async$self.enqueueFrame$3(frameObj, controller, finalBuffer);
        ...
        while (true)
          switch ($async$goto) {
            ...
            case 2:
              ...
              t7 = $.$get$logger();
              t7.log$4(B.Level_FINER_400, string$.decodex3ad + A.NativeUint8List_NativeUint8List$view(decrypted, 0, null).length, null, null);
              t3 = t6.decrypted;
              if (t3 == null)
                throw A.wrapException(A.Exception_Exception("[decryptFrameInternal] could not decrypt"));
              t7.log$4(B.Level_FINER_400, string$.decodex3ad + A.NativeUint8List_NativeUint8List$view(t3, 0, null).length, null, null);
              $async$goto = t10.currentkeySet !== t6.initialKeySet ? 3 : 4;
              break;
            case 3:
              // then
              t7.log$4(B.Level_FINE_500, string$.decodex3ar, null, null);
              $async$goto = 5;
  ...
  A.KeyProvider.prototype = {
    ...
    setSharedKey$2$keyIndex(key, keyIndex) {
      var $async$goto = 0,
        $async$completer = A._makeAsyncAwaitCompleter(type$.void),
        $async$self = this;
      var $async$setSharedKey$2$keyIndex = A._wrapJsFunctionForAsync(function($async$errorCode, $async$result) {
        if ($async$errorCode === 1)
          return A._asyncRethrow($async$result, $async$completer);
        while (true)
          switch ($async$goto) {
            case 0:
              // Function start
              $.$get$logger().log$4(B.Level_INFO_800, "setting shared key", null, null);
              $async$self.sharedKey = key;
              $async$goto = 2;
              return A._asyncAwait($async$self.getSharedKeyHandler$0().setKey$2$keyIndex(key, keyIndex), $async$setSharedKey$2$keyIndex);
            case 2:
              // returning from await.
              // implicit return
              return A._asyncReturn(null, $async$completer);
      ...
      return A._asyncStartSync($async$setSharedKey$2$keyIndex, $async$completer);
    }
  };
  A.KeySet.prototype = {};
  ...
  A.NALUProcessingResult.prototype = {};
  ...
  A.main_closure0.prototype = {
    call$1($event) {
      ...
      cryptor.setupTransform$7$codec$isReuse$kind$operation$readable$trackId$writable(codec == null ? _null : codec, false, kind, msgType, t2, trackId, t1);
    },
    $signature: 12
  };
        ...
        while (true)
          switch ($async$goto) {
            ...
            case 7:
              ...
              for (t2 = cryptors.length, t3 = "" + enabled, t4 = "Set enable " + t3 + " for trackId ", t5 = "setEnabled[" + t3 + string$.x5d__las, _i = 0; _i < t2; ++_i) {
                cryptor = cryptors[_i];
            ...
            case 9:
              ...
              A._asString(kind);
              $async$goto = 25;
              return A._asyncAwait(cryptor.setupTransform$6$isReuse$kind$operation$readable$trackId$writable(exist && msgType === "decode", kind, msgType, readable, trackId, writable), $async$call$1);
            case 25:
              // returning from await.
            ...
            case 26:
              ...
              $async$goto = 29;
              return A._asyncAwait(keyProvider.setSharedKey$2$keyIndex(key, keyIndex), $async$call$1);
            case 29:
              // returning from await.
              // goto join
              $async$goto = 27;
            ...
            case 17:
              ...
              t2 = "" + keyIndex;
              $async$goto = keyProvider.keyProviderOptions.sharedKey ? 36 : 38;
              break;
            case 36:
              // then
            ...
            case 21:
              ...
              if (A.IterableExtension_firstWhereOrNull(B.List_Algorithm_0_Algorithm_1, new A.main__closure3(algorithmStr), type$.Algorithm) == null) {
                type$.JSObject._as(self.self).postMessage(A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["type", "dataCryptorEncrypt", "error", "algorithm not found", "msgId", msgId, "msgType", "response"], type$.String, type$.nullable_String)));
                // goto return
                $async$goto = 1;
                break;
              }
              ...
              if (keyProvider == null) {
                t1.log$4(B.Level_WARNING_900, "KeyProvider not found for " + keyProviderId, null, null);
                type$.JSObject._as(self.self).postMessage(A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["type", "dataCryptorEncrypt", "error", "KeyProvider not found", "msgId", msgId, "msgType", "response"], type$.String, type$.nullable_String)));
                // goto return
                $async$goto = 1;
                break;
              }
            ...
            case 45:
              ...
              type$.JSObject._as(self.self).postMessage(A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["type", "dataCryptorEncrypt", "participantId", participantId, "dataCryptorId", dataCryptorId, "data", encryptedPacket.data, "keyIndex", encryptedPacket.keyIndex, "iv", encryptedPacket.iv, "msgId", msgId, "msgType", "response"], type$.String, type$.nullable_Object)));
              $async$handler = 2;
              // goto after finally
              $async$goto = 44;
              break;
            case 42:
              ...
              t1 = self;
              type$.JSObject._as(t1.self).postMessage(A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["type", "dataCryptorEncrypt", "error", J.toString$0$(e0), "msgId", msgId, "msgType", "response"], type$.String, type$.nullable_String)));
              // goto after finally
              $async$goto = 44;
              break;
            case 41:
            ...
            case 22:
              ...
              if (A.IterableExtension_firstWhereOrNull(B.List_Algorithm_0_Algorithm_1, new A.main__closure4(algorithmStr), type$.Algorithm) == null) {
                type$.JSObject._as(self.self).postMessage(A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["type", "dataCryptorDecrypt", "error", "algorithm not found", "msgId", msgId, "msgType", "response"], type$.String, type$.nullable_String)));
                // goto return
                $async$goto = 1;
                break;
              }
              ...
              if (keyProvider == null) {
                t1.log$4(B.Level_WARNING_900, "KeyProvider not found for " + keyProviderId, null, null);
                type$.JSObject._as(self.self).postMessage(A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["type", "dataCryptorDecrypt", "error", "KeyProvider not found", "msgId", msgId, "msgType", "response"], type$.String, type$.nullable_String)));
                // goto return
                $async$goto = 1;
                break;
              }
            ...
            case 50:
              ...
              type$.JSObject._as(self.self).postMessage(A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["type", "dataCryptorDecrypt", "participantId", participantId0, "dataCryptorId", dataCryptorId0, "data", decryptedData, "msgId", msgId, "msgType", "response"], type$.String, type$.nullable_Object)));
              $async$handler = 2;
              // goto after finally
              $async$goto = 49;
              break;
            case 47:
              ...
              t1 = self;
              type$.JSObject._as(t1.self).postMessage(A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["type", "dataCryptorDecrypt", "error", J.toString$0$(e1), "msgId", msgId, "msgType", "response"], type$.String, type$.nullable_String)));
              // goto after finally
              $async$goto = 49;
              break;
            case 46:
            ...
            case 23:
              ...
              type$.JSObject._as(self.self).postMessage(A.jsify(A.LinkedHashMap_LinkedHashMap$_literal(["type", "dataCryptorDispose", "dataCryptorId", dataCryptorId, "msgId", msgId, "msgType", "response"], type$.String, type$.nullable_String)));
              // goto after switch
              $async$goto = 4;
              break;
            case 24:
  ...
  (function installTearOffs() {
    ...
    var _;
    _instance_2_u(_ = A.FrameCryptor.prototype, "get$encodeFunction", "encodeFunction$2", 10);
    _instance_2_u(_, "get$decodeFunction", "decodeFunction$2", 10);
  })();
  (function inheritance() {
    ...
    _inheritMany(A.Object, [A.JS_CONST, J.Interceptor, J.ArrayIterator, A._CopyingBytesBuilder, A.Error, A.SentinelValue, A.Iterable, A.ListIterator, A.MappedIterator, A.WhereIterator, A.FixedLengthListMixin, A.TypeErrorDecoder, A.NullThrownFromJavaScriptException, A.ExceptionAndStackTrace, A._StackTrace, A.Closure, A.MapBase, A.LinkedHashMapCell, A.LinkedHashMapKeyIterator, A._UnmodifiableNativeByteBufferView, A.Rti, A._FunctionParameters, A._Type, A._TimerImpl, A._AsyncAwaitCompleter, A.AsyncError, A.Stream, A._BufferingStreamSubscription, A._BroadcastStreamController, A._Completer, A._FutureListener, A._Future, A._AsyncCallbackEntry, A._DelayedEvent, A._PendingEvents, A._DoneStreamSubscription, A._StreamIterator, A._Zone, A._HashMapKeyIterator, A.ListBase, A.Codec, A.Converter, A._Base64Encoder, A._Base64Decoder, A.DateTime, A._Enum, A.OutOfMemoryError, A.StackOverflowError, A._Exception, A.FormatException, A.Null, A._StringStackTrace, A.StringBuffer, A.NullRejectionException, A._JSSecureRandom, A.Level, A.LogRecord, A.Logger, A.EncryptedPacket, A.E2EEDataPacketCryptor, A.FrameInfo, A.FrameCryptor, A.KeyOptions, A.KeyProvider, A.KeySet, A.ParticipantKeyHandler, A.NALUProcessingResult, A.SifGuard]);
    ...
    _inheritMany(A.Closure, [A.Closure0Args, A.Closure2Args, A.TearOffClosure, A.initHooks_closure, A.initHooks_closure1, A._AsyncRun__initializeScheduleImmediate_internalCallback, A._AsyncRun__initializeScheduleImmediate_closure, A._awaitOnObject_closure, A._SyncBroadcastStreamController__sendData_closure, A._Future__chainForeignFuture_closure, A._Future__propagateToListeners_handleWhenCompleteCallback_closure, A.Stream_length_closure, A.jsify__convert, A.promiseToFuture_closure, A.promiseToFuture_closure0, A.dartify_convert, A.getTrackCryptor_closure, A.getDataPacketCryptor_closure, A.unsetCryptorParticipant_closure, A.unsetDataPacketCryptorParticipant_closure, A.main_closure, A.main_closure0, A.main_closure2, A.main__closure, A.main__closure0, A.main__closure1, A.main__closure2, A.main__closure3, A.main__closure4, A.main_closure1]);
    _inheritMany(A.TearOffClosure, [A.StaticClosure, A.BoundClosure]);
    _inherit(A._AssertionError, A.AssertionError);
    _inheritMany(A.MapBase, [A.JsLinkedHashMap, A._HashMap]);
    _inheritMany(A.Closure2Args, [A.initHooks_closure0, A._awaitOnObject_closure0, A._wrapJsFunctionForAsync_closure, A._Future__chainForeignFuture_closure0, A._Future__propagateToListeners_handleWhenCompleteCallback_closure0, A.MapBase_mapToString_closure]);
    ...
    _inheritMany(A._Enum, [A.Algorithm, A.CryptorError]);
...
;
    ...
    B.CryptorError_4 = new A.CryptorError("kUnsupportedCodec");
    ...
    B.List_Algorithm_0_Algorithm_1 = A._setArrayType(makeConstList([B.Algorithm_0, B.Algorithm_1]), A.findType("JSArray<Algorithm>"));
