...
///
/// Run with: flutter test integration_test/production_services_test.dart -j 1
/// Storage is created in temp directory and auto-cleaned by OS
void main() {
  TestWidgetsFlutterBinding.ensureInitialized();
  group('Production Signal Services Integration Tests', () {
    // CRITICAL: Tests must run sequentially (not in parallel)
    // FlutterSecureStorage file locking prevents concurrent access
    setUp(() async {
    ...
    tearDown(() async {
      // CRITICAL: Wait for background async operations to complete
      // PermanentSignedPreKeyStore auto-generates keys in constructor
      // These .then() callbacks must finish before next test starts
    ...
    testWidgets('SignalKeyManager - prekey generation and retrieval', (
      tester,
    ) async {
      final keyManager = await SignalKeyManager.create();
    ...
    testWidgets('EncryptionService - full stack initialization', (
      tester,
    ) async {
      final keyManager = await SignalKeyManager.create();
      final sessionManager = await SessionManager.create(
    ...
    testWidgets('EncryptionService - basic encryption operations', (
      tester,
    ) async {
      final keyManager = await SignalKeyManager.create();
      final sessionManager = await SessionManager.create(
    ...
    testWidgets('Storage isolation - multiple device identities', (
      tester,
    ) async {
      // First identity
      await initializeTestDeviceIdentity(
